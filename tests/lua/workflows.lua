return function()
  ...
  local ok, err = pcall(function()
    workflows:open()
    settle()
    assert(vim.treesitter.language.get_lang("agent-manager-agents") == "markdown")
    ...
    assert(text():find("\n---\n", 1, true), "workflow programs have Markdown separators")
    assert(not text():find("├", 1, true) and not text():find("└", 1, true), "flat workflow rows")
